...
class RptRenjaBelanjaTakLangsungController extends Controller
{
    ...
    public function getDataReport()
    {
        ...
        $idRenjaAngg = $this->request->query->get("id_renja_anggaran");
        ...
        $idBidang = $this->request->query->get("sikd_bidang_id");
        $formatRpt = $this->request->query->get("format");
        $param = [
            'id_renja' => $idRenja,
            'id_renja_anggaran' => $idRenjaAngg,
            'tahun' => $tahun,
...
//         return $renjaReports;
        ...
        $i = 0;
        foreach ($renjaRepHandler as $renjaSikdBidId) {
            $mapRenjaSikdBidangId[$i] =  $renjaSikdBidId->renja_anggaran_sikd_bidang_id;
            $i++;
        }
        ...
        $ids = implode(',', $param2["sikd_bidang_id"]); $parentParam = '&parents=3';
        $paramIn = ['ids' => $ids];$paramIn ['parents']=3;
        $sikdRenja = $this->restClient->getCollection("$tahun/sikdbidangs", $paramIn);
        $mapSikdBidang = $this->populateSikdBidangInfo($sikdRenja);
        
//         return $mapSikdRekRincObj;
        $renjaAnggaranForm = array();
        foreach ($renjaReports as &$value1) {
            $value1->sikd_satker_id_sikd_satker = $idSatker;
            $value1->sikd_satker_kode = $kdSatker;
            $value1->sikd_satker_nama = $nmSatker;
            $value1->sikd_sub_skpd_id_sikd_sub_skpd = $idSubUnit;
